...
FROM
	estado e;-- somente as colunas nome e sigla serão apresentadas
...
SELECT nome,sigla,id FROM estado WHERE nome = 'PARANA'; -- mostra nome e sigla do estado que possui o nome igual a PARANÁ
...
FROM
	estado
WHERE
	YEAR(estado.data_cadastro)=2020;
...
FROM
	estado
WHERE
	YEAR(estado.data_cadastro)!=2020; -- pode ser YEAR(data_cadastro) != 2020;
...
FROM
	estado
WHERE
YEAR(estado.data_cadastro)=2016
OR YEAR(estado.data_cadastro)=2018;
    
-- Consultas com 3 condições na cláusula WHERE
-- consultando estados que foram cadastrados em 2015, 2017 OU 2020
SELECT
	estado.nome 'NOME ESTAOD'
    ,estado.sigla 'SIGLA ESTAOD'
    ,estado.data_cadastro 'DATA DE CADASTRO'
FROM
	estado
WHERE
	YEAR(estado.data_cadastro)=2016
    OR YEAR(estado.data_cadastro)=2017
	OR YEAR(estado.data_cadastro)=2018;
...
FROM
	estado
WHERE
	YEAR(data_cadastro) IN(2016,2017,2018);
...
SELECT
	nome,sigla,data_cadastro,ativo
FROM
	estado
WHERE
	YEAR(data_cadastro) BETWEEN 2015 AND 2019
