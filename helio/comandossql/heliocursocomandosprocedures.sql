...
DROP PROCEDURE verificar_par_impar;
DELIMITER //
CREATE PROCEDURE verificar_par_impar(numero INT, OUT resultado VARCHAR (100))
BEGIN 
    IF MOD(numero, 2) = 0 THEN 
		SET resultado = 'PAR';
	ELSE 
...
CALL verificar_par_impar(4,@meu_resultado);
SELECT @meu_resultado;

SET @nome_completo = concat('Clayson ','Caitano');
