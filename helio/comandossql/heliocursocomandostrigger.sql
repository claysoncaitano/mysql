...
UPDATE cliente SET nome = 'MARTA FERNANDA' WHERE ID = 2; 
OLD.nome (MARTA) NEW.nome (MARTA FERNANDA) 

INSERT INTO cliente (nome,cpf) VALUES ('JOCA','888.888.888-88');
OLD.nome ??? NÃO EXISTE NEW.nome ?? JOCA

DELETE FROM cliente WHERE id = 1;
OLD.nome RAUL  NEW.nome ?? NÃO EXISTE
*/
DROP TRIGGER IF EXISTS validar_alteracao_cliente_inativo;
DELIMITER //
...
SELECT * FROM conta_receber WHERE cliente_id = 8;

SELECT * FROM conta_receber;

UPDATE cliente SET nome = 'MARTA FERNANDA' WHERE id = 2;

UPDATE cliente SET ativo = 'N' WHERE id = 8;
