...
CALL inserir_endereco('DOMENIQUE DANESE','12');
...
CREATE PROCEDURE inserir_endereco(IN rua VARCHAR(255), IN numero INT)
BEGIN
	DECLARE EXIT HANDLER FOR SQLEXCEPTION SELECT 'NÃO FOI POSSÍVEL CONTINUAR O CADASTRO' AS MESSAGE;
	INSERT INTO endereco (rua, numero) VALUES (rua, numero);
END
// DELIMITER ;

CALL inserir_endereco('DOMENIQUE DANESE','18');
CALL inserir_endereco(NULL,'18');
SELECT * FROM endereco;
...
DELIMITER //
CREATE PROCEDURE inserir_cliente(nome_cliente VARCHAR(255), nome_rua VARCHAR(255), numero_rua INT)
BEGIN
...
CALL inserir_cliente('MARIA','DR. FRANCISCO',71);
